# MSc Cyber Security e-Portfolio

**University of Essex**

This repository documents my learning journey throughout the **MSc Cyber Security** programme. It brings together module activities, coding exercises, practical artefacts, collaborative discussions, testing work, and reflections on the knowledge and skills developed throughout the programme.

The e-Portfolio will continue to be updated as new modules and units are completed.

## Portfolio Links

| Resource | Link |
|---|---|
| **Live e-Portfolio** | [Open e-Portfolio](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/) |
| **GitHub Repository** | [Browse Repository](https://github.com/Ralemadi/MSc-Cyber-Security-e-Portfolio) |

---

# Modules

## Advanced Object-Oriented Design and Programming

This module explores advanced object-oriented principles, secure software design, SOLID principles, and creational, structural and behavioural design patterns through practical programming activities and reflective work.

**[View Module e-Portfolio Overview](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop.html)**  
**[Browse Module Repository](advanced-oop/)**

### Units

| Unit | Topic | e-Portfolio | Repository |
|---|---|---|---|
| **Unit 1** | Introduction and Recap of Object-Oriented Programming (OOP) | [View Unit](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop/unit-1/unit-1.html) | [Browse Files](advanced-oop/unit-1/) |
| **Unit 2** | SOLID Principles of Object-Oriented Design | [View Unit](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop/unit-2/unit-2.html) | [Browse Files](advanced-oop/unit-2/) |
| **Unit 3** | Design Patterns I – Creational Patterns | [View Unit](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop/unit-3/unit-3.html) | [Browse Files](advanced-oop/unit-3/) |
| **Unit 4** | Design Patterns II – Structural Patterns | [View Unit](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop/unit-4/unit-4.html) | [Browse Files](advanced-oop/unit-4/) |
| **Unit 5** | Design Patterns III – Behavioural Patterns | [View Unit](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop/unit-5/unit-5.html) | [Browse Files](advanced-oop/unit-5/) |
| **Unit 6** | Concurrency and Parallelism in Object-Oriented Design | [View Unit](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop/unit-6/unit-6.html) | [Browse Files](advanced-oop/unit-6/) |
| **Unit 7** | Secure Coding Practices in Object-Oriented Programming | [View Unit](https://ralemadi.github.io/MSc-Cyber-Security-e-Portfolio/advanced-oop/unit-7/unit-7.html) | [Browse Files](advanced-oop/unit-7/) |
### Module Documentation

The module repository contains individual unit pages, practical exercises, source code, screenshots, supporting documents and reflections.

**[View Advanced OOP Module README](advanced-oop/README.md)**

---

## Future Modules

Additional MSc Cyber Security modules will be added to this repository as they are completed.
